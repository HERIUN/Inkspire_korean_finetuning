    ...
    def __init__(
        ...
        if add_downsample:
            self.downsamplers = nn.ModuleList(
                [
                    Downsample2D(
                        out_channels, use_conv=True, out_channels=out_channels, padding=downsample_padding, name="op", vertical_total_compression=vertical_total_compression
                    )
                ]
            )
        else:
...
class UpDecoderBlock2D(nn.Module):
    def __init__(
        ...
        if add_upsample:
            self.upsamplers = nn.ModuleList([Upsample2D(out_channels, use_conv=True, out_channels=out_channels, vertical_total_compression=vertical_total_compression)])
        else:
            self.upsamplers = None
