...
| 파일 | 변경 |
|---|---|
| `infer/show.py` :137, :197 | `load_model("inkspire:<lora_dir>[,<layout_ckpt>]")` 디스패치(지연 import) + `gen_from_style` 이 `.gen` 있는 모델은 그대로 호출 → `eval/htr_cer.py`, `experiments/gen_compare.py` 무수정으로 두 백엔드 비교 |
| `eval/htr_cer.py` | `--style-ref-text`(non-echo one-shot 프로토콜), 폰트 glob `*.[ot]tf`(custom_hw 는 otf) |
| `configs/loader.py` :46 | `PATH_KEYS` 에 `empty_cache`, `lora_dir`, `layout_ckpt` |
| `configs/eval.yaml`, `configs/infer.yaml` | `htr_cer.style_ref_text`, `inkspire:` 섹션 |
...
마스크는 그 아래 전부. 생성 후 목표 줄의 상자 범위만 잘라 HTR 에 넘긴다 — `eval/htr_cer.py` 와 `gen_compare` 가 받는 것도 이 한 줄이다.
...
② **레이아웃 정확도 자체도 논문보다 나쁘다.** 논문 표 1 의 CFM 은 L1×10³ 로 Δx 4.74 / Δy 14.39 / Δw 4.74 / Δh 4.94 (평균 **7.2e-3**). 우리 s8000 은 11.7px / page_w 1024 = **11.4e-3** — 약 1.6배. 20k 완주와 EMA 를 안 한 상태다.
...
| 2 | **레이아웃 모델 20k 완주 + EMA**, Δy 가중 | 9.2 ② 논문 대비 1.6배 오차. Δy 가 가장 큰 항 | 6h |
